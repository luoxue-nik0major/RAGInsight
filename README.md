...
### 4. 查询复杂度分析与策略推荐
- **五维度复杂度评分**：查询长度、句子数、实体密度、关系词密度、语义复杂度、跳数需求
- **自动问题类型识别**：事实型 / 定义型 / 列表型 / 比较型 / 因果型 / 多跳型
- **策略推荐引擎**：根据复杂度自动推荐向量检索 / 混合检索 / 图检索
...
- **图检索**：基于实体共现图的多跳 BFS 扩展，适合复杂关系查询，返回带推理路径的 Chunk

### 6. 批量实验与评估
- 预置多维度测试数据集：英文 SQuAD + 中文唐诗宋词（各 30+ 条查询）
- **实验一**：故障检测准确率评估（Precision / Recall / F1，按类型与宏平均）
- **实验二**：跨检索架构对比（向量 vs 混合，对比相关性、覆盖率、多样性、答案质量、耗时）
- **实验三**：单条会话 JSON 导出 + Markdown 实验报告一键生成
- 实验面板支持中英文数据集切换

### 7. 工程化保障
- **48+ 自动化测试**：pytest 覆盖核心服务与端到端流程
...
| 方法 | 端点 | 说明 |
|------|------|------|
| `POST` | `/api/query` | SSE 流式执行 RAG 查询（事件：step / alert / root_cause / done） |
...
项目已完成 **6 个 Phase** 的迭代开发，是一个功能完整的 RAG 可视化诊断原型系统。

| 维度 | 评估 | 说明 |
|------|------|------|
| 功能完整性 | ★★★★☆ | 全链路闭环：查询 → 检索 → 质量评估 → 根因诊断 → 扰动分析 → 答案生成 |
| 代码质量 | ★★★★☆ | 分层架构清晰，设计模式得当，TypeScript + Pydantic 双端类型安全 |
| 测试覆盖 | ★★★★☆ | 48+ pytest 用例覆盖核心模块，全部通过 |
| 工程化 | ★★★☆☆ | 缺少 Docker/Alembic/CI，任务队列为内存级 |
| 论文支撑 | ★★★☆☆ | 缺自动评估指标、PDF 报告导出、更大规模数据集 |

### 关键指标

| 指标 | 数据 |
|------|------|
| 后端代码量 | ~2,928 行 Python（22 个源文件） |
| 前端代码量 | ~2,277 行 TypeScript/TSX（12 个源文件） |
| 测试代码量 | ~537 行（7 个文件，48+ 用例） |
| 检索策略 | 3 种（向量 / BM25+向量 RRF 混合 / networkx 图检索） |
| 故障检测类型 | 6 种（空结果/低相关性/低覆盖率/低多样性/上下文超长/幻觉风险） |
| 支持语言 | 中文 + 英文（双语复杂度分析、中文分词） |
| 测试数据集 | 英文 30 条 + 中文 30 条 |

### 待改进项

1. **安全**：需轮换已暴露的 API Key，完善 `.env.example` 模板
2. **评估体系**：答案质量仅靠"引用有效性"间接判断，缺少 LLM-as-a-Judge 自动指标（答案忠实度/相关性评分）
3. **实验数据**：测试集偏小（共 60 条），论文需要更大规模标注数据（目标 100+ 条）
4. **报告导出**：缺 PDF 实验报告导出，图表无法直接用于论文
5. **可视化**：缺时间轴回放、答案-Chunk 双向联动高亮
6. **工程化**：无 Docker 部署配置、无 Alembic 数据库迁移
...
| 优先级 | 任务 | 周次 | 工作量 | 价值 |
|--------|------|------|--------|------|
| **P0** | 安全与配置治理（API Key 轮换、`.env.example`、配置分层） | W1 | 0.5d | 代码可公开 |
| **P0** | LLM-as-a-Judge 自动评估指标（答案忠实度+相关性） | W1 | 3-4d | 论文核心数据 |
| **P1** | PDF 实验报告导出（html2canvas + jspdf） | W2 | 1.5d | 论文图表直接可用 |
| **P1** | 测试数据集扩展（60→100+ 条，含标注） | W2 | 2d | 实验可靠性 |
| **P2** | 流程图时间轴回放 + 慢步骤高亮 | W3 | 2d | 可视化亮点 |
| **P2** | 答案句子 ↔ Chunk 双向联动高亮 | W3 | 1.5d | 演示核心亮点 |
| **P2** | Docker 部署配置（docker-compose 一键启动） | W3 | 1d | 降低演示风险 |
| **P2** | Alembic 数据库迁移 | W3 | 0.5d | 工程化基础 |

> 总计约 12-13 天工作量。详细计划见 `C:\Users\lenovo\.claude\plans\structured-wibbling-mitten.md`

---

## 📌 已知限制

1. **图检索依赖预建图**：`graph` 策略需要预先运行 `scripts/build_knowledge_graph.py` 构建实体共现图，否则自动退化为向量检索。
2. **并发控制**：异步任务队列（扰动分析、批量实验）使用内存级信号量，服务重启后任务状态丢失。
3. **单用户部署**：暂无用户认证与多租户隔离，适合本地开发/演示场景。
4. **前端 API 路径硬编码**：axios 未配置 baseURL，部署到非本地环境需手动修改。
